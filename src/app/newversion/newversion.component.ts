...
export class NewversionComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    const workMin = <HTMLSpanElement>document.querySelector("#work-min");
    ...
    for (var key in incrementFunctions) {
        if (incrementFunctions.hasOwnProperty(key)) {
          (<HTMLButtonElement>document.querySelector(key)).onclick = incrementFunctions[key];
        }
    }
    ...
    function updateHTML() {
      ...
      workMin.textContent = workTime.toString();
      breakMin.textContent = breakTime.toString();  
    }
